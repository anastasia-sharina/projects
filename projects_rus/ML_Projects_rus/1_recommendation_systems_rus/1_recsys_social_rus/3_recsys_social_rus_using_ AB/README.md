# A/B сервис рекомендаций

Описание
- A/B сервис реализует распределение пользователей на контрольную и тестовую группы (A/B) и выдаёт персональные рекомендации с использованием двух независимых моделей (control / test).
...
- Служит демонстрацией лучших практик: централизованное подключение к БД через общий модуль `0_recsys_movies_ru_common/db_connect.py` (маскировка credentials), читабельные логи.
...
Метрики качества
- Hitrate, ROC-AUC — основная метрика для оценки ранжирования.
- Feature importance — анализ вклада признаков в модель.

Безопасность
- Все учетные данные для подключения к БД читаются в одном месте: `0_recsys_movies_ru_common/db_connect.py`.
...
- В логах не выводятся полные переменные окружения и строки подключения. Логируются только безопасные сведения.
